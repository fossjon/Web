...
function msgdec(mode)
{
	...
	if (i < mail.length)
	{
		...
		if (mode == "list")
		{
			...
			if (newa != "") { coln = ("<td class='colmin'><span class='glyphicon glyphicon-certificate txtgreen' style='top: 4px;'></span></td>"); }
			if (colt.length > 1) { colb = ("<td class='colmin'><span class='glyphicon glyphicon-file' style='top: 3px; color: #555555;'></span></td>"); }
			if (seclist.length > 2) { colc = ("<td class='colmin'><img src='"+webp+"/img/lock.png' style='margin-top: 2px;' /></td>"); }
			
			var cold = tmpinf[2];
			if (view == "o") { cold = tmpinf[3]; }
			
			rdecdata += ("<tr>"+cola+colc+colb+coln+tmpinf[1]+cold+tmpinf[4]+"</tr>\n");
		...
		if (mode == "view")
		{
			...
			jQuery('#from').html("<a href='"+webp+"/make/?a="+tmpinf[2]+"'>"+tmpinf[2]+"</a>");
			...
			for (var j in rlist)
			{
				if (j > 0)
				{
					var adata = window.atob(rlist[j]);
					var idata = adata.indexOf(" ");
					var alist = [adata.slice(0, idata), adata.slice(idata + 1)];
					if (alist.length > 1)
					{
						var ativr = alist[1].substring(0, 16);
						var atkey = aesdec(ativr, tmpkey, alist[1].substring(16, 48));
						jQuery('#attach').append("<a href='"+webp+"/file.php?i="+ativr.strTOhex()+"&k="+atkey.strTOhex()+"&e="+mail[i][5]+"&f="+alist[0]+"'><span class='glyphicon glyphicon-file'></span> <span id='attach"+j+"'></span></a> &nbsp; &nbsp; ");
